...
export class LigaFormAdminUnrouted implements OnInit {
  ...
  openEquipoFinderModal(): void {
    ...
    dialogRef.afterClosed().subscribe((equipo: IEquipo | null) => {
      if (equipo) {
        ...
        this.syncEquipo(equipo.id ?? null);
  ...
  onSubmit(): void {
    ...
    // Validar que se haya seleccionado equipo
    if (!selectedEquipoId) {
    ...
    const ligaData: any = {
      ...
      equipo: { id: Number(selectedEquipoId) },
    };
  ...
  private saveCreate(ligaData: any): void {
    this.oLigaService.create(ligaData).subscribe({
      ...
      error: (err: HttpErrorResponse) => {
        // Workaround: Si el error es 500 por serialización JSON pero la liga se creó, tratarlo como éxito
        if (err.status === 500 && err.error?.message?.includes('Could not write JSON')) {
          this.snackBar.open('Liga creada exitosamente', 'Cerrar', { duration: 4000 });
          this.submitting.set(false);
          this.formSuccess.emit();
          return;
